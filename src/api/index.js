...
export const login = (username, password) => {
    ...
    return axios.post('http://192.168.1.2:8000/login', data)
}

export const fetchExampleLists = () => {
    return axios.get('http://192.168.1.2:8000/forms')
}

export const fetchDetail = (slug) => {
    return axios.get(`http://192.168.1.2:8000/question/?slug=${slug}`)
}

export const getPoint = (slug) => {
    return axios.get(`http://192.168.1.2:8000/point?slug=${slug}`)
}

export const sendPhoto = (uniq_slug, example_slug, file) => {
    if(file.platform == 'android') {
        return axios.post('http://192.168.1.2:8000/quickstart/upload-photo', {
            uniq_slug: uniq_slug,
    ...
    return fetch('http://192.168.1.2:8000/quickstart/upload-photo', {
        method: 'post',
        body: bodyFormData,
        headers: {
